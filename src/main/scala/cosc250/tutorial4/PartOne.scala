...
object PartOne extends App {
	...
	implicit class VecOps(val vec:Vec2D) extends AnyVal {
		...
		def +(other:Vec2D):Vec2D = (vec._1 + other._1,vec._2 + other._2)

		def -(other:Vec2D):Vec2D = (vec._1 - other._1,vec._2 - other._2)

		def *(scalar:Double):Vec2D = (scalar * vec._1, scalar * vec._2)

		def /(scalar:Double):Vec2D = (vec._1 / scalar, vec._2 / scalar)

		def magnitude:Double = Math.sqrt(Math.pow(vec._1, 2) + Math.pow(vec._2, 2))

		def angle:Double = Math.atan(vec._2  / vec._1)
